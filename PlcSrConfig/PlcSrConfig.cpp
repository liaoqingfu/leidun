    ...
    QWidget(parent),ui(new Ui::PlcSrConfig)
{
    ui->setupUi(this);
    loadPlcConfig();
    this->resize(170,340);//重新窗口设置大小，源始大小为170*340
...
void PlcSrConfig::savePlcConfig()
{
    ...
    else
    {
        ...
        for (int i = 0; i <plcList.count(); i++)
        {
            ...
            rwLen=rwLenMap[pid];
            qDebug()<<portId<<baudRate<<parity<<dataBits<<stopBits<<type<<rwDelay<<rwLen;
            //再写入保存
...
void PlcSrConfig::saveTempConfig()
{
    ...
    qDebug()<<"保存"<<plcName<<"传感器临时配置"<<portId<<baudRate<<parity<<dataBits<<stopBits<<type<<rwDelay<<rwLen;
}
void PlcSrConfig::addDefaultConfig()
{
    ...
    {//简单判断输入
        if(QMessageBox::Yes==QMessageBox::question(this,"请选择",QString("是否添加%1传感器？").arg(plcName),QMessageBox::Yes|QMessageBox::No|QMessageBox::Yes))
        {
            ...
            rwLen=QString("%1").arg(Lth);
            rwLenMap[plcName]=rwLen;
            qDebug()<<"添加新的传感器"<<plcName<<"加载默认配置"<<portId<<baudRate<<parity<<dataBits<<stopBits<<type<<rwDelay<<rwLen;
            return;
        }else
            qDebug()<<"已放弃添加";
        return;
